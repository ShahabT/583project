...
public class main {
	...
	public static void main(String[] args) throws Exception {
		...
		try{
			...
			resultSet = statement.executeQuery("SELECT user_tip.name, MAX(user_tip.likes) FROM (SELECT name, likes FROM user INNER JOIN tip on user.user_id = tip.user_id) AS user_tip GROUP BY user_tip.name;");
			while(resultSet.next()){
